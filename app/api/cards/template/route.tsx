  ...
  try {
    ...
    while (finding) {
      ...
      if (randomStatlines.length !== 3 || randomTypes.length !== 3) {
        return new Response(
          JSON.stringify({ message: "Issue Generating Types || Statlines" }),
          { status: 409 }
        );
      }
      ...
      await Promise.all(
        randomTypes.map(async (type: any) => {
      ...
      await Promise.all(
        randomTypes.map(async (type: any) => {
          let imageOptions = await UnclaimedImage.find({
            type: type.prompt,
            alterations: type.alterations,
            promptVersion: {
              $in: ["1.12", "1.11"],
            },
          });
          ...
          if (number_of_images < 3) {
            ...
            imageOptions = imageOptions.map((image) => {
              let new_image = image;
              new_image.photo = image.photo.replace(
      ...
      await Promise.all(
        randomTypes.map(async (type: any) => {
          ...
          const alteration =
            type.alterations === "null" ? "Base" : type.alterations;

          let availableEffects =
            // @ts-ignore
            cardEffects[creatureType]?.[alteration]?.effects || [];
          ...
          for (let i = 0; i < 3; i++) {
            if (availableEffects.length > 0) {
              const randomIndex = Math.floor(
                Math.random() * availableEffects.length
              );
    ...
    return new NextResponse(
      JSON.stringify({ data: returnData, headers: headers }),
      { status: 200 }
    );
  } catch (error) {
    return new Response(
...
const getWeightedRandom = (input: any, number: number) => {
  ...
  for (let i = 0; i < number; i++) {
    const totalWeight = items.reduce(
      (total: number, item: any) => total + parseFloat(item.probability),
      0
    );
